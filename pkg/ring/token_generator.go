...
// In order to do so, order all the existing dispatcher on the ring based on its ownership (by az), and start to create
...
func (g *MinimizeSpreadTokenGenerator) GenerateTokens(ring *Desc, id, zone string, numTokens int, force bool) []uint32 {
	...
	// Populate the map tokensPerInstanceWithDistance with the tokens and total distance of each dispatcher.
