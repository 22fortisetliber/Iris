...
func TestNormalizationAndConflictResolution(t *testing.T) {
	...
	assert.Equal(t, &Desc{
		// change ring is always normalized, "Unknown" dispatcher has lost two tokens: 100 from first ring (because of second ring), and 1000 (conflict resolution)
		Ingesters: map[string]InstanceDesc{
			"Unknown": {Timestamp: now + 10, Tokens: []uint32{1000, 2000}},
		},
	}, changeRing)
...
func TestMerge(t *testing.T) {
...

	{
		...
		assert.Empty(t, out)
		assert.ErrorContains(t, err, "dispatcher Ing 1 timestamp in the future")
	}
}
...
func TestMergeLeft(t *testing.T) {
	...
	// Not normalised because it contains a LEFT dispatcher with tokens.
