...
func TestLifecycler_JoinShouldNotBlock(t *testing.T) {
	...
	l1, err := NewLifecycler(lifecyclerConfig1, &nopFlushTransferer{}, "dispatcher", ringKey, false, true, log.NewNopLogger(), nil)
...
func TestLifecycler_DefferedJoin(t *testing.T) {
	...
	// Add the  dispatcher to the ring with deferred join
	// Add the  dispatcher to the ring with autojoin
	lifecyclerConfig1 := testLifecyclerConfig(ringConfig, "ing1")
	lifecyclerConfig1.HeartbeatPeriod = 100 * time.Millisecond
	...
	l1, err := NewLifecycler(lifecyclerConfig1, &nopFlushTransferer{}, "dispatcher", ringKey, false, true, log.NewNopLogger(), nil)
	require.NoError(t, err)
	l2, err := NewLifecycler(lifecyclerConfig2, &nopFlushTransferer{}, "dispatcher", ringKey, true, true, log.NewNopLogger(), nil)
	require.NoError(t, err)
...
func TestLifecycler_HealthyInstancesCount(t *testing.T) {
	...
	// Add the first dispatcher to the ring
	lifecyclerConfig1 := testLifecyclerConfig(ringConfig, "ing1")
	lifecyclerConfig1.HeartbeatPeriod = 100 * time.Millisecond
	lifecyclerConfig1.JoinAfter = 100 * time.Millisecond

	lifecycler1, err := NewLifecycler(lifecyclerConfig1, &nopFlushTransferer{}, "dispatcher", ringKey, true, true, log.NewNopLogger(), nil)
	require.NoError(t, err)
	...
	// Assert the first dispatcher joined the ring
	test.Poll(t, 1000*time.Millisecond, true, func() interface{} {
		return lifecycler1.HealthyInstancesCount() == 1
	})

	// Add the second dispatcher to the ring
	lifecyclerConfig2 := testLifecyclerConfig(ringConfig, "ing2")
	lifecyclerConfig2.HeartbeatPeriod = 100 * time.Millisecond
	lifecyclerConfig2.JoinAfter = 100 * time.Millisecond

	lifecycler2, err := NewLifecycler(lifecyclerConfig2, &nopFlushTransferer{}, "dispatcher", ringKey, true, true, log.NewNopLogger(), nil)
	require.NoError(t, err)
	...
	// Assert the second dispatcher joined the ring
	test.Poll(t, 1000*time.Millisecond, true, func() interface{} {
		return lifecycler2.HealthyInstancesCount() == 2
	})

	// Assert the first dispatcher count is updated
	test.Poll(t, 1000*time.Millisecond, true, func() interface{} {
...
func TestLifecycler_ZonesCount(t *testing.T) {
	...
	for idx, event := range events {
		...
		// Register an dispatcher to the ring.
		cfg := testLifecyclerConfig(ringConfig, fmt.Sprintf("instance-%d", idx))
		cfg.HeartbeatPeriod = 100 * time.Millisecond
		cfg.JoinAfter = 100 * time.Millisecond
		cfg.Zone = event.zone

		lifecycler, err := NewLifecycler(cfg, &nopFlushTransferer{}, "dispatcher", ringKey, true, true, log.NewNopLogger(), nil)
		require.NoError(t, err)
...
func TestLifecycler_NilFlushTransferer(t *testing.T) {
	...
	lifecycler, err := NewLifecycler(lifecyclerConfig, nil, "dispatcher", ringKey, true, true, log.NewNopLogger(), nil)
	require.NoError(t, err)
...
func TestLifecycler_ShouldHandleInstanceAbruptlyRestarted(t *testing.T) {
	...
	r, err := New(ringConfig, "dispatcher", ringKey, log.NewNopLogger(), nil)
	require.NoError(t, err)
	require.NoError(t, services.StartAndAwaitRunning(context.Background(), r))
	defer services.StopAndAwaitTerminated(context.Background(), r) //nolint:errcheck

	// Add an 'dispatcher' with normalised tokens.
	lifecyclerConfig1 := testLifecyclerConfig(ringConfig, "ing1")
	l1, err := NewLifecycler(lifecyclerConfig1, &nopFlushTransferer{}, "dispatcher", ringKey, true, true, log.NewNopLogger(), nil)
	require.NoError(t, err)
	require.NoError(t, services.StartAndAwaitRunning(context.Background(), l1))

	// Check this dispatcher joined, is active, and has one token.
	test.Poll(t, 1000*time.Millisecond, true, func() interface{} {
	...
	// Add a second dispatcher with the same settings, so it will think it has restarted
	l2, err := NewLifecycler(lifecyclerConfig1, &nopFlushTransferer{}, "dispatcher", ringKey, true, true, log.NewNopLogger(), nil)
	require.NoError(t, err)
	require.NoError(t, services.StartAndAwaitRunning(context.Background(), l2))

	// Check the new dispatcher picked up the same tokens and registered timestamp.
	test.Poll(t, 1000*time.Millisecond, true, func() interface{} {
		d, err := r.KVClient.Get(context.Background(), ringKey)
		require.NoError(t, err)
...
// Ensure a check ready returns error when consul returns a nil key and the dispatcher already holds keys. This happens if the ring key gets deleted
func TestCheckReady_NoRingInKVStore(t *testing.T) {
	...
	l1, err := NewLifecycler(cfg, &nopFlushTransferer{}, "dispatcher", ringKey, true, true, log.NewNopLogger(), nil)
	require.NoError(t, err)
	require.NoError(t, services.StartAndAwaitRunning(ctx, l1))
	t.Cleanup(func() {
...
func TestRestartIngester_DisabledHeartbeat_unregister_on_shutdown_false(t *testing.T) {
	...
	// Starts dispatcher and wait it to became active
	...
	// We are going to create 2 fake dispatcher with disabled heart beat and `unregister_on_shutdown=false` then
	// test if the dispatcher 2 became active after:
	...
	// Both dispatcher should be active and running
	assert.Equal(t, ACTIVE, ingesters["ing1"].State)
	assert.Equal(t, ACTIVE, ingesters["ing2"].State)

	// Stop One dispatcher gracefully should leave it on LEAVING STATE on the ring
...
func TestTokensOnDisk(t *testing.T) {
	...
	l1, err := NewLifecycler(lifecyclerConfig, &noopFlushTransferer{}, "dispatcher", ringKey, true, true, log.NewNopLogger(), nil)
	require.NoError(t, err)
	require.NoError(t, services.StartAndAwaitRunning(context.Background(), l1))

	// Check this dispatcher joined, is active, and has 512 token.
	...
	// Start new dispatcher at same token directory.
	lifecyclerConfig.ID = "ing2"
	l2, err := NewLifecycler(lifecyclerConfig, &noopFlushTransferer{}, "dispatcher", ringKey, true, true, log.NewNopLogger(), nil)
	require.NoError(t, err)
	require.NoError(t, services.StartAndAwaitRunning(context.Background(), l2))
