...
func TestInstanceDesc_IsHealthy_ForIngesterOperations(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"ACTIVE dispatcher with last keepalive newer than timeout": {
			dispatcher:     &InstanceDesc{State: ACTIVE, Timestamp: time.Now().Add(-30 * time.Second).Unix()},
			timeout:        time.Minute,
			writeExpected:  true,
			readExpected:   true,
			reportExpected: true,
		},
		"ACTIVE dispatcher with last keepalive older than timeout": {
			dispatcher:     &InstanceDesc{State: ACTIVE, Timestamp: time.Now().Add(-90 * time.Second).Unix()},
			timeout:        time.Minute,
			writeExpected:  false,
			readExpected:   false,
			reportExpected: false,
		},
		"JOINING dispatcher with last keepalive newer than timeout": {
			dispatcher:     &InstanceDesc{State: JOINING, Timestamp: time.Now().Add(-30 * time.Second).Unix()},
			timeout:        time.Minute,
			writeExpected:  false,
			readExpected:   true,
			reportExpected: true,
		},
		"LEAVING dispatcher with last keepalive newer than timeout": {
			dispatcher:     &InstanceDesc{State: LEAVING, Timestamp: time.Now().Add(-30 * time.Second).Unix()},
			timeout:        time.Minute,
			writeExpected:  false,
			readExpected:   true,
			reportExpected: true,
	...
	for testName, testData := range tests {
		...
		t.Run(testName, func(t *testing.T) {
			actual := testData.dispatcher.IsHealthy(Write, testData.timeout, time.Now())
			assert.Equal(t, testData.writeExpected, actual)

			actual = testData.dispatcher.IsHealthy(Read, testData.timeout, time.Now())
			assert.Equal(t, testData.readExpected, actual)

			actual = testData.dispatcher.IsHealthy(Reporting, testData.timeout, time.Now())
...
func TestDesc_Ready(t *testing.T) {
	...
	if err := r.IsReady(now.Add(5*time.Minute), 10*time.Second); err == nil {
		t.Fatal("expected !ready (no heartbeat from active dispatcher), but got no error")
	}
	...
	r.Ingesters["some dispatcher"] = InstanceDesc{
		Tokens:    []uint32{12345},
		Timestamp: now.Unix(),
	}
